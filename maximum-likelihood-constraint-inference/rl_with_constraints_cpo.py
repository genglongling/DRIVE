#!/usr/bin/env python3
"""
RL with Constraints - CPO (Constrained Policy Optimization) Version
This version uses CPO for constraint satisfaction.
"""
...
class CPOConstraintAwarePolicyNetwork(nn.Module):
    """
    Policy network that uses CPO for constraint satisfaction.
    """
    def __init__(self, state_dim=23, action_dim=4, hidden_dim=128, constraint_model=None):
        super(CPOConstraintAwarePolicyNetwork, self).__init__()
    ...
    def forward(self, state, constraint_mask=None):
        """
        Forward pass with CPO for constraint satisfaction.
        """
        # Get raw action logits
        action_logits = self.policy_net(state)
        value = self.value_net(state)
        
        # Apply CPO constraint satisfaction
        if constraint_mask is not None:
            action_logits = self._apply_cpo_constraints(state, action_logits, constraint_mask)
    ...
    def _apply_cpo_constraints(self, state, action_logits, constraint_mask):
        """
        Apply CPO to satisfy constraints.
        """
        try:
            # Get CPO constraint costs for each action
            constraint_costs = []
            for action in range(self.action_dim):
                ...
                # Check constraint satisfaction using CPO approach
                if self.constraint_model is not None:
                    constraint_prob = self.constraint_model(next_state)
                    # CPO uses cost function (1 - probability) for constraint violation
                    constraint_cost = 1.0 - constraint_prob.item()
                    constraint_costs.append(constraint_cost)
                else:
                    constraint_costs.append(0.0)
            
            # Adjust action logits based on CPO constraint costs
            constraint_costs = torch.tensor(constraint_costs).to(state.device)
            # CPO penalizes actions with high constraint costs
            adjusted_logits = action_logits - constraint_costs.unsqueeze(0) * 10.0
            ...
            print(f"CPO constraint application failed: {e}, using fallback")
            return action_logits

class CPOConstraintGuidedRL:
    """
    RL agent that uses CPO for constraint satisfaction.
    """
    def __init__(self, state_dim=23, action_dim=9, constraint_model_path=None, 
        ...
        self.policy_network = CPOConstraintAwarePolicyNetwork(
            state_dim=state_dim, 
        ...
        print(f"✓ CPO Constraint-Guided RL initialized")
    ...
    def select_action(self, state, training=True):
        """
        Select action using CPO for constraint satisfaction.
        """
        ...
        # Get action probabilities with CPO
        with torch.no_grad():
    ...
    def _apply_action_to_state(self, state, action):
        """Apply action to state using CPO transition model."""
        dt = 0.4  # Time step
        ...
        # Use CPO transition model if available
        try:
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(self.device)
            ...
            # Predict next state using CPO transition model
            ...
            print(f"CPO transition failed: {e}, using fallback")
    ...
    def compute_reward(self, state, action, next_state, df=None, frame=None, goal_pos=None):
        """Compute reward with CPO-based considerations."""
        reward = 0.0
        ...
        # 4. CPO constraint satisfaction bonus
        try:
            # Bonus for actions that satisfy CPO constraints
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(self.device)
            action_one_hot = torch.zeros(1, self.action_dim).to(self.device)
            action_one_hot[0, action] = 1.0
            state_action = torch.cat([state_tensor, action_one_hot], dim=1)
            
            # Predict next state using CPO model
            next_state_pred = self.policy_network.transition_net(state_action)
            
            # Check if predicted state satisfies constraints
            if self.policy_network.constraint_model is not None:
                constraint_prob = self.policy_network.constraint_model(next_state_pred)
                if constraint_prob.item() > 0.8:  # Good CPO constraint satisfaction
                    reward += 5.0
        except:
            pass
    ...
    def update_policy(self, batch_size=32):
        """Update policy using CPO techniques."""
        ...
        # Compute loss with CPO regularization
        loss = F.mse_loss(current_q.squeeze(), target_q)
        
        # Add CPO constraint loss
        ...
        # CPO constraint loss: penalize constraint violations
        if self.policy_network.constraint_model is not None:
            constraint_probs = self.policy_network.constraint_model(predicted_next_states)
            constraint_loss = F.mse_loss(constraint_probs, torch.ones_like(constraint_probs))
        else:
            constraint_loss = torch.tensor(0.0).to(self.device)
        
        # Combined loss with CPO constraint penalty
    ...
    def train_episode(self, initial_state, df, max_steps=100, goal_pos=None):
        """Train for one episode using CPO."""
    ...
    def save_model(self, filepath):
        ...
        print(f"✓ CPO RL model saved to {filepath}")
    ...
    def load_model(self, filepath):
        ...
        print(f"✓ CPO RL model loaded from {filepath}")
    ...
    def save_trajectory_data(self, output_dir="./trajectory_data"):
        ...
        training_path = os.path.join(output_dir, "cpo_rl_training_history.csv")
        ...
        summary_path = os.path.join(output_dir, "cpo_rl_episode_summary.csv")
        df_summary.to_csv(summary_path, index=False)
        
        print(f"✓ CPO RL trajectory data saved to {output_dir}")
        return training_path, summary_path

def create_cpo_rl_training_data(csv_file, constraint_model_path=None, num_episodes=1000):
    """Create and train a CPO constraint-guided RL agent."""
    print(f"=== Creating CPO Constraint-Guided RL Training Data ===")
    ...
    # Initialize RL agent
    rl_agent = CPOConstraintGuidedRL(
        constraint_model_path=constraint_model_path,
        num_episodes=num_episodes
    )
    
    # Training loop
    print(f"\nTraining CPO RL agent for {num_episodes} episodes...")
    ...
    print(f"✓ CPO RL training completed")
    return rl_agent

def generate_cpo_rl_trajectories(rl_agent, start_state, df, max_steps=100):
    """Generate trajectories using the trained CPO RL agent."""
...
if __name__ == "__main__":
    ...
    # Create and train CPO RL agent
    rl_agent = create_cpo_rl_training_data(
        csv_file=csv_file,
        constraint_model_path=constraint_model_path,
        num_episodes=500
    )
    ...
    # Save trained agent
    rl_agent.save_model("./rl_agent_trained_cpo.pth")
    
    # Save trajectory data
    rl_agent.save_trajectory_data("./trajectory_data")
    
    print("✓ CPO RL with constraints training completed!") 