...
def minimise_jerk_scp(
    ...
    for i in range(scp_iterations):
        ...
        objective = cp.Minimize(cp.sum_squares(ax[1:] - ax[:-1]) +
                            cp.sum_squares(ay[1:] - ay[:-1]))
...
def test_planner_with_csv_2d(
    ...
    """
    Tests the 2D planner (minimise_jerk_2d or minimise_jerk_2d) using real vehicle data.
    """
    ...
    for front_track_id,unique_ego_trackId in zip(unique_front_ids, unique_ego_trackId):
        ...
        try:
            if objective == 'jerk':
                print("Objective: Minimizing jerk...")
                filename=f"plots/min_jerk/trackcsv_{track_num}/trajectory_track_{front_track_id}.pdf"
                planner_result = minimise_jerk_scp(
    ...
    output_csv_path = f"jerk_data/track_{track_num}/all_trajectories.csv"
