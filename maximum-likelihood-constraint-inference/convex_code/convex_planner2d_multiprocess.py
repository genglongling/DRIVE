...
def minimise_time_scp(
    ...
    while left <= right:
        ...
        # --- SCP Iteration Loop ---
        s_ref_traj = None # Start with no reference trajectory
        ...
        for i in range(scp_iterations):
            ...
            if prob.status in (cp.OPTIMAL, cp.OPTIMAL_INACCURATE):
                ...
                if i == scp_iterations - 1:
                    is_feasible_for_T = True
                    s_val = s_ref_traj
        ...
        if is_feasible_for_T:
            best_solution = (T, s_val, v_val, a_val)
            right = T - 1  # Feasible, try smaller T
        else:
            left = T + 1   # Infeasible, need more time
...
def minimise_time(
    ...
    while left <= right:
        ...
        for t in range(T - 1):
            ...
            # Simple distance constraint to lead vehicle
            s_front_t = s_front_traj[t]
            constr += [
                cp.norm(cp.hstack([sx[t] - s_front_t[0], sy[t] - s_front_t[1]])) >= d_min
            ]
        ...
        if prob.status in (cp.OPTIMAL, cp.OPTIMAL_INACCURATE):
            s_val = np.column_stack((sx.value, sy.value))
            v_val = np.column_stack((vx.value, vy.value))
            a_val = np.column_stack((ax.value, ay.value))
            best_solution = (T, s_val, v_val, a_val)
            right = T - 1  # Feasible, try smaller T
        else:
            left = T + 1   # Infeasible, need more time
...
def test_planner_with_csv_2d(
    ...
    """
    Tests the 2D planner (minimise_time_2d or minimise_effort_2d) using real vehicle data.
    """
    ...
    for front_track_id,unique_ego_trackId in zip(unique_front_ids, unique_ego_trackId):
        ...
        try:
            if objective == 'time':
                print("Objective: Minimizing Time...")
                filename=f"plots/min_time/trackcsv_{track_num}//trajectory_track_{front_track_id}.pdf"
                planner_result = minimise_time_scp(
                    s_init, v_init, a_init, s_front_traj, V_MAX, A_MAX, D_MIN,
                    goal_center, T_min=1, T_max=min(2*T_real_horizon,2000), Δ_t=Δt, static_obs=None
                )
                if planner_result:
            ...
            # Store data
            s_front_T = to_len(s_front_traj, T_star)
            v_front_T = to_len(v_front_traj, T_star)
            a_front_T = to_len(a_front_traj, T_star)

            s_real_T  = to_len(s_real_traj,  T_star)
            v_real_T  = to_len(v_real_traj,  T_star)
            a_real_T  = to_len(a_real_traj,  T_star)
            ...
            trajectory_data = pd.DataFrame({
                ...
                'lead_pos_x': s_front_T[:, 0], 'lead_pos_y': s_front_T[:, 1],
                'lead_vel_x': v_front_T[:, 0], 'lead_vel_y': v_front_T[:, 1],
                'lead_acc_x': a_front_T[:, 0], 'lead_acc_y': a_front_T[:, 1],
                'real_pos_x': s_real_T[:, 0],  'real_pos_y': s_real_T[:, 1],
    ...
    output_csv_path = f"time_data/track_{track_num}/all_trajectories.csv"
