...
def minimise_distance_scp(
    ...
    for i in range(scp_iterations):
        ...
        segment_costs = []
        for t in range(T - 1):
            Δs = cp.hstack([sx[t + 1] - sx[t],   # x‑component
                            sy[t + 1] - sy[t]])  # y‑component
            segment_costs.append(cp.norm(Δs, 2)) # ‖·‖₂ on the 2‑D displacement

        total_travel_distance = cp.sum(cp.hstack(segment_costs))
        objective= cp.Minimize(total_travel_distance)
...
def test_planner_with_csv_2d(
    ...
    """
    Tests the 2D planner (minimise_distance_2d or minimise_effort_2d) using real vehicle data.
    """
    ...
    for front_track_id,unique_ego_trackId in zip(unique_front_ids, unique_ego_trackId):
        ...
        try:
            if objective == 'distance':
                print("Objective: Minimizing distance...")
                filename=f"plots/min_distance/trackcsv_{track_num}/trajectory_track_{front_track_id}.pdf"
                planner_result = minimise_distance_scp(
    ...
    output_csv_path = f"distance_data/track_{track_num}/all_trajectories.csv"
